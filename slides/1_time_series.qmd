...
* **ISO date-time string**, for example `2026-03-09T08:15:30.125Z`
...
```{r}
sensor_df %>%
  mutate(minute_bin = floor_date(timestamp, "10 minutes"),
         hour = hour(timestamp))
```

```{r}
sensor_df %>%
  filter(timestamp >= ymd_hms("2026-03-09 08:05:00", tz = "UTC"),
         timestamp <  ymd_hms("2026-03-09 08:25:00", tz = "UTC"))
```
:::
:::
...
sensor_xts2 <- xts::xts(x = sensor_tbl2$temperature,
...
db.measurements.find({
  timestamp: {
    $gte: ISODate("2026-03-09T08:00:00.000Z"),
    $lt:  ISODate("2026-03-09T09:00:00.000Z")
  }
}).sort({ timestamp: 1 })
```
...
m <- mongo(collection = "measurements",
           db = "plant",
...
df <- m$find(query = query, fields = fields,
             sort = '{"timestamp": 1}')
```
...
sensor_df %>%
  ...
  geom_point() +
  labs(x = "Timestamp", y = "Value",
       title = "Selected time window ready for analysis")
```
