...
subtitle: "Multi-agent system for smart machining, part I-d"
...
\end{bmatrix},
~~~
\mathbf{t}_t^T = \mathbf{x}_{c,t}^T \mathbf{P},~~~\mathbf{P} = [\mathbf{p}_1,\mathbf{p}_2,\dots,\mathbf{p}_p]
$$
...
::: {.columns}
::: {.column}
For centered observation row $\mathbf{x}_{c,t}^T$, PCA computes the score row
$$
\mathbf{t}_t^T = \mathbf{x}_{c,t}^T \mathbf{P}_a
$$
...
$$
\mathbf{x}_{c,t}^T = \hat{\mathbf{x}}_{c,t}^T + \boldsymbol{\varepsilon}_t^T
$$
where $\hat{\mathbf{x}}_{c,t}^T$ lies in the retained **latent subspace** and $\boldsymbol{\varepsilon}_t^T$ is the residual row
:::
::: {.column}
...
\textrm{subject to:}\quad y_i(\mathbf{w}^T\mathbf{x}_i-b)\ge1,\quad i=1,\dots,N
$$
...
:::
::: {.column width="40%"}
![Maximum-margin hyperplane in 2-D](https://upload.wikimedia.org/wikipedia/commons/thumb/7/72/SVM_margin.png/960px-SVM_margin.png)

:::
:::
<!-- end columns -->

## SVM: Interactive Example in $\mathbb R^2$

::: {.columns}
::: {.column}
:::smaller
* we search for two **parallel** (same $\mathbf w = \mathbf w(\theta, b)$) lines (i.e. 2-D hyperplanes) that separate the two classes and are as far apart as possible
* the zone between them is called the **margin**
* the **support vectors** are those that satisfy $y_i(\mathbf{w}^T\mathbf{x}_i-b) = 1$
...
\textrm{subject to:}\quad y_i(\mathbf{w}^T\mathbf{x}_i-b)\ge1-\xi_i
$$
...
::: {.callout-note}
The LD formulation of any minimization problem is a maximization problem and *vice versa*. The dual is a quadratic function subject to linear constraints, which is **easier to solve** than the original problem. 
:::
...
::: {.columns}
::: {.column}
Instead of optimizing the hyperplane parameters directly, the dual problem optimizes one coefficient $\alpha_i$ for each training sample:
$$
\max_{\boldsymbol{\alpha}}
\sum_{i=1}^{N}\alpha_i -
\frac{1}{2}\sum_{i=1}^{N}\sum_{j=1}^{N}
...
For the soft-margin SVM, the constraints are
$$
...
::: {.columns}
::: {.column}
In the dual problem and in the decision function, the samples appear only through inner products:
$$
\mathbf{x}_i^T\mathbf{x}_j
$$

If we first map the data into a feature space $\phi(\mathbf{x})$, the same algorithm would use
$$
...
$$
where only the **support vectors** $x_i,~i \in SV$ contribute
...
:::column
```{r}
ts_df %>% ggplot(aes(x = x1, y = x2, color = state)) +
  geom_point() +
  labs(x = "x1", y = "x2", title = "Original feature space")
```
:::
:::
...
```{r}
#| echo: TRUE
pca_fit <- prcomp(~x1+x2, data=ts_df, center = TRUE, scale. = TRUE)
summary(pca_fit)
round(pca_fit$rotation, 3)
scores <- as_tibble(pca_fit$x) %>% mutate(t = ts_df$t, state = ts_df$state)
```
We also calculate $\mathbf \lambda$ and rescale the data:
...
ggplot() +
  ...
  geom_point(data = ts_df,
             ...
             size = 1.8) +
  labs(x = "x1", y = "x2",
       title = paste("SVM decision accuracy:", round(acc, 3)))
```
:::
:::
...
p2 <- ts_df %>% 
  ...
  geom_line() +
  scale_color_manual(values = c("x1" = "blue", "x2" = "orange")) +
  theme(legend.position = "none") 
...
df = pd.DataFrame({"t": t, "x1": x1, "x2": x2, "state": state})
```
...
Look at the [exercise notebook](/notebooks/exercise_03.qmd){target="_blank"} for exercises on PCA and SVM
...
# Exercises
Look at the [exercise notebook](/notebooks/exercise_03.qmd){target="_blank"} for exercises on PCA and SVM analysis in R, Python, and MongoDB.