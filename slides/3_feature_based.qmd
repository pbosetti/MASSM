...
In feature-based analysis, a time series is transformed into a sequence of vectors
$$
...
$$
where each row is a feature vector extracted at one time instant or from one time window

PCA starts from the centered matrix
$$
...
* $\mathbf{p}_i$ is the corresponding loading vector

The loadings are sorted so that
$$
...
::: {.columns}
::: {.column}
The projection of the data onto the principal axes gives the **scores**
$$
...
$$
\eta_a = \frac{\sum_{i=1}^{a}\lambda_i}{\sum_{i=1}^{p}\lambda_i}
$$
...
::: {.columns}
::: {.column}
For process monitoring, PCA separates the observation into:
$$
\mathbf{x}_t = \hat{\mathbf{x}}_t + \mathbf{e}_t
$$
where $\hat{\mathbf{x}}_t$ lies in the retained latent subspace and $\mathbf{e}_t$ is the residual
:::
::: {.column}
