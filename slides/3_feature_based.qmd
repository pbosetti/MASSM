...
::: {.columns}
::: {.column}
For observation vector $\mathbf{x}_{c,t}$, PCA computes the score vector
$$
...
We calculate the residuals vector $\mathbf\varepsilon = \mathbf{x}_c - \hat{\mathbf{x}}_c$ and the monitoring statistics. Given the $\lambda$ values, we only keep $\mathbf{p}_1$



```{r}
#| echo: TRUE
tcrossprod(pca_fit$x, pca_fit$rotation)
